...
currentSubsampleRate <- 0.51
currentColsampleRate <- 0.7
ntreesNum <- 2800
min_child_weight_num <- 1.91
gamma_num <- 0.05
...
# ntrees : 1500 / shrinkage : 0.01 / n.minobsinnode : 19 / interaction.depth : 48 / rmse : 23518.88 -> LB : 

ntreesNum <- 1300 # rmseErrorsHyperparameters_gbm[nrow(rmseErrorsHyperparameters_gbm), 2]
shrinkage_num <- 0.01 # rmseErrorsHyperparameters_gbm[nrow(rmseErrorsHyperparameters_gbm), 3]
n.minobsinnode_num <- 19 # rmseErrorsHyperparameters_gbm[nrow(rmseErrorsHyperparameters_gbm), 4]
interaction.depth_num <- 49 # rmseErrorsHyperparameters_gbm[nrow(rmseErrorsHyperparameters_gbm), 5]
...
## RandomForest : RMSE : 24316.66 -> LB : 0.14295 -------------
...
# 참고 - http://www.aistudy.co.kr/neural/theory_oh.htm#_bookmark_23d2610
...
epochs_num = 1000 # seq(200, 600, by = 10)
...
for (i in 265:nrow(grid_search_keras)) {
    set.seed(1234)
    rm(model, history)
    ...
    history <- model %>%
				fit(as.matrix(X_train[train.ind,]), as.matrix(train_XY[train.ind, 82]), epochs = epochs_num, batch_size = batch_size_num, verbose = verbose_num, validation_split = validation_split_num, callback_early_stopping(monitor = "val_loss", mode = "min", patience = 1)) # patience  = val_loss가 min이 아니더라도 1분동안은 계속 진행

    score <- model %>% evaluate(as.matrix(X_train[-train.ind,]), train_XY[-train.ind, 82])
    sqrt(score$loss) #rmse

    rmse <- sqrt(score$loss) #rmse

    key <- toString(i)
    rmseErrorsHyperparameters_keras_1 <- data.frame(i, rmse, input_activation, hidden1_activation, hidden2_activation, output_activation, batch_size_num, epochs_num)
...
write.csv(pred, paste0("C:/Users/bevis/Downloads/House_Prices_Advanced_Regression_Techniques/Ensemble_model_ave_", format(Sys.time(), "%Y%b%d_%H%M%S"), ".csv ", sep = " "), quote = F, row.names = F)
...
## GBM old : RMSE : 25048.87 -> LB : 0.12840 -------------
...
gbmFit <- train(SalePrice ~ ., method = "gbm", metric = "RMSE", maximize = FALSE,
				...
				tuneGrid = expand.grid(n.trees = c(350, seq(2000, 2500, by = 100)), interaction.depth = c(5), shrinkage = c(0.05), n.minobsinnode = c(10)),
